...
def generate_mock_data(columns, n=300):
    ...
    first_names_mae = [
        "Maria", "Ana", "Julia", "Fernanda", "Beatriz", "Gabriela", "Mariana", 
        "Camila", "Patricia", "Aline", "Sandra", "Regina", "Adriana", "Amanda",
        "Jose" # 14 nomes femininos + 1 nome masculino
    ]
    ...
    for col in columns:
        if col == 'contador':
            ...
            data[col] = [f"{random.choice(first_names_mae)} {random.choice(last_names)} {random.choice(last_names)}" for _ in range(n)]
        elif col == 'NOME':
...
def generate_full_pipeline(n, sim_out, sinasc_out, matches_out):
    sim_df = generate_mock_data(sim_columns, n)
    sinasc_df = generate_mock_data(sinasc_columns, n)
    ...
    sim_df.to_csv(sim_out, index=False, sep=';')
    sinasc_df.to_csv(sinasc_out, index=False, sep=';')
    true_matches.to_csv(matches_out, index=False, sep=';')

# Generate both the test and the 100-row versions
generate_full_pipeline(300, 'sim_test.csv', 'sinasc_test.csv', 'matches_test.csv')
generate_full_pipeline(100, 'sim_100.csv', 'sinasc_100.csv', 'matches_100.csv')
